...
export default function StartQuizPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleAnswerSelect = (answerId: number) => {
    ...
    setSelectedAnswers(prev => 
      prev.map(ans => 
        ans.questionId === currentQuestionId ? { ...ans, answerId: answerId } : ans
      )
    );
  };
  ...
  const handleSubmitTest = async () => {
    if (!testId || !studentName.trim()) {
      alert("Proszę podać swoje imię przed wysłaniem!");
      return;
    }
  ...
  if (error) {
     return (
       <div className="min-h-screen flex flex-col items-center justify-center bg-black p-4">
         <div className="bg-zinc-900 border border-red-500/50 p-12 rounded-[2rem] text-center max-w-md">
            <h2 className="text-3xl font-black text-red-500 mb-4 uppercase">Błąd</h2>
            <p className="text-zinc-400 mb-8">{error}</p>
            <Link href="/" className="bg-zinc-800 text-white font-bold py-3 px-8 rounded-xl hover:bg-zinc-700 transition uppercase text-xs">Powrót</Link>
         </div>
       </div>
     );
  }
    ...
    return (
      <div className="max-w-2xl mx-auto px-4 py-20">
        <div className="bg-zinc-900 border-2 border-yellow-400/30 p-12 rounded-[3rem] shadow-2xl text-center">
          <h1 className="text-5xl font-black text-white uppercase italic mb-6 tracking-tighter">
            quiz: <span className="text-yellow-400">"{quizTopic}"</span>
          </h1>
          
          <div className="mb-8 text-left">
            <label className="text-[10px] uppercase font-black tracking-widest text-zinc-500 mb-2 block pl-4">Twoje Imię</label>
            <input 
              type="text" 
              value={studentName}
          ...
          <button 
            onClick={() => studentName.trim() ? setIsStarted(true) : alert("Podaj imię!")}
  ...
  if (submissionResult) {
         ...
         <div className="max-w-4xl mx-auto px-4 py-12 animate-in fade-in duration-500">
           ...
           <div className="space-y-8">
             {questions.map((q) => {
               ...
               return (
                 <div key={q.id} className="bg-zinc-900 border border-zinc-800 p-8 rounded-[2rem] shadow-xl">
                   <div className="flex items-start justify-between gap-4 mb-6">
                       <p className="text-white text-xl font-bold leading-relaxed tracking-tight">{q.text}</p>
                       <div className={`shrink-0 w-8 h-8 rounded flex items-center justify-center font-black ${isCorrectlyAnswered ? 'bg-green-500 text-black' : 'bg-red-500 text-white'}`}>
                           {isCorrectlyAnswered ? '✓' : '✕'}
                       </div>
                   </div>
                   ...
                   <div className="grid grid-cols-1 gap-3">
                     {q.answers.map((ans) => {
                       ...
                       return (
                         <div key={ans.id} className={`p-4 rounded-xl border-2 flex justify-between items-center ${style}`}>
    ...
    return (
        <div className="max-w-2xl mx-auto px-4 py-20 text-center text-white">
            <h1 className="text-6xl font-black mb-4 uppercase italic tracking-tighter">Wynik: {submissionResult.score}/{submissionResult.max_score}</h1>
            <p className="text-2xl text-zinc-500 mb-10 font-bold uppercase tracking-widest">{submissionResult.percentage.toFixed(0)}%</p>
            ...
            <div className="flex flex-col gap-4">
              ...
              <Link href="/" className="bg-yellow-400 text-black px-12 py-5 rounded-[2rem] font-black uppercase tracking-widest border-b-8 border-yellow-600 active:scale-95 transition-all">
                Koniec
              </Link>
            </div>
        </div>
  ...
  if (isReviewingBeforeSubmit) {
    ...
    return (
      <div className="max-w-2xl mx-auto px-4 py-20 animate-in fade-in zoom-in-95 duration-500">
        <div className="bg-zinc-900 border border-zinc-800 p-10 rounded-[3rem] shadow-2xl text-center">
          <h2 className="text-4xl font-black text-white uppercase italic mb-8 tracking-tighter">Gotowy do wysłania?</h2>
          ...
          <div className="bg-zinc-950 rounded-2xl p-6 mb-10 border border-zinc-800">
              ...
              <p className={`text-4xl font-black ${answeredCount === questions.length ? 'text-green-400' : 'text-yellow-400'}`}>
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-12">
      ...
      <div className="bg-zinc-900 border border-zinc-800 p-8 sm:p-16 rounded-[2.5rem] shadow-2xl relative">
        <h2 className="text-2xl sm:text-3xl font-black text-white text-center leading-relaxed mb-12 italic tracking-tight">{currentQuestion?.text}</h2>
        <div className="grid grid-cols-1 gap-3">
          {currentQuestion?.answers.map((ans) => {
            ...
            return (
              ...
              >
                {ans.text}
                <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center transition-all ${isSelected ? 'border-yellow-400 bg-yellow-400' : 'border-zinc-700 group-hover:border-zinc-500'}`}>
      ...
      <div className="flex justify-between mt-10 gap-4">
          <button onClick={handlePrev} disabled={currentIdx === 0} className="px-8 py-5 bg-zinc-800 text-white rounded-xl disabled:opacity-30 font-black uppercase text-[10px] tracking-widest transition-all">Wstecz</button>
          <button onClick={handleNext} className="px-12 py-5 bg-yellow-400 text-black rounded-xl font-black border-b-4 border-yellow-600 uppercase text-[10px] tracking-widest active:scale-95 transition-all">
