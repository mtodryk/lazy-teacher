...
export default function MyQuizesPage() {
  ...
  useEffect(() => {
    ...
    const fetchQuizes = async () => {
      try {
        const res = await fetch('http://localhost:8000/api/quizes/', {
          headers: {
        ...
        // Fetch document titles for all quizes
        const uniqueDocIds = [...new Set(data.map((t: QuizItem) => t.document_id))] as number[];
  ...
  const toggleSubmissions = async (quizId: number) => {
    ...
    if (submissions[quizId]) return;

    setLoadingSubmissions(quizId);
    try {
      const res = await fetch(`http://localhost:8000/api/quizes/${quizId}/submissions/`, {
        headers: {
          Authorization: `Token ${token}`,
          'Content-Type': 'application/json'
        }
      ...
      setSubmissions((prev) => ({ ...prev, [quizId]: data }));
    } catch (err) {
      console.error(err);
    } finally {
  ...
  const copyQuizLink = (quiz: QuizItem) => {
    const link = `${window.location.origin}/start-quiz/${quiz.code}`;
    ...
    <div className="max-w-5xl mx-auto px-4 py-12 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      {quizes.length === 0 ? (
        <div className="text-center py-20">
          ...
          <p className="text-zinc-500 text-lg font-medium mb-6">Nie masz jeszcze żadnych quizów.</p>
          <Link
            href="/upload"
        ...
        <div className="space-y-6">
          {quizes.map((quiz) => {
            ...
            const isLoadingSubs = loadingSubmissions === quiz.id;
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-3 mb-2">
                        <h2 className="text-xl md:text-2xl font-black text-white uppercase italic tracking-tight truncate">
                          {quiz.questions[0]?.topic || quiz.code}
                        </h2>
                        <span
                          className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest ${quiz.is_active ? 'bg-green-500/20 text-green-400' : 'bg-zinc-700/50 text-zinc-500'}`}
                        >
                      ...
                      <div className="flex items-center gap-4 text-zinc-500 text-sm font-medium">
                        ...
                        <span>•</span>
                        <span>{new Date(quiz.created_at).toLocaleDateString('pl-PL')}</span>
                      </div>
                    </div>

                    <div className="flex items-center gap-3 flex-shrink-0">
                      <Link
                        href={`/quiz-setup/${quiz.id}`}
                      ...
                      <button
                        onClick={() => toggleSubmissions(quiz.id)}
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-6 pt-6 border-t border-zinc-800">
                      ...
                      <div className="flex items-center justify-between gap-3 p-4 bg-green-500/5 border border-green-500/30 rounded-xl">
                        <div className="flex items-center gap-3 min-w-0">
                          ...
                          <div className="min-w-0">
                            ...
                            <p className="text-sm font-bold text-green-400 truncate">{docTitles[quiz.document_id]}</p>
                          </div>
                        </div>
                        <button
                          onClick={() => handleDownloadDocument(quiz.document_id)}
                          disabled={downloadingDocId === quiz.document_id}
                          className="flex-shrink-0 text-green-400 hover:text-green-300 transition-colors disabled:opacity-50"
                          title="Pobierz dokument"
                        >
                          {downloadingDocId === quiz.document_id ? (
                    ...
                    <div className="flex items-center justify-between gap-3 p-4 bg-yellow-400/5 border border-yellow-400/30 rounded-xl">
                      <div className="flex items-center gap-3 min-w-0">
                        ...
                        <div className="min-w-0">
                          <p className="text-xs text-zinc-500 font-medium">Link do quizu</p>
                          <a
                            href={`${typeof window !== 'undefined' ? window.location.origin : ''}/start-quiz/${quiz.code}`}
                            target="_blank"
                      ...
                      <button
                        ...
                        className={`flex-shrink-0 transition-colors ${copiedQuizId === quiz.id ? 'text-green-400' : 'text-yellow-400 hover:text-yellow-300'}`}
                        title={copiedQuizId === quiz.id ? 'Skopiowano!' : 'Kopiuj link'}
                      >
